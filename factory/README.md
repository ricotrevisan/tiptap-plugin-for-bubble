...
| In Review + `ship-approved` | Ship session | Refreshes the PR if main moved (re-gated and re-reviewed), guarded merge, `pled push` of only this ticket, check on `tiptap-demo`, lifecycle lab checks before and after the push (a new failure blocks), baseline commit, Done, removes the worktree. |
