...
## Where to work
- Worktree (use only this): $worktree. A new branch starts from origin/main at $base_sha.
- Rework: if the branch already has commits or an open PR, this is a rework round. The maintainer moved the reviewed ticket back to the queue. Read their latest comments on the ticket and PR, then continue that same PR. Merge origin/main into the branch first if it's behind.
...
- Keep the ticket truthful with `$linear_ticket_cli`, run from the worktree:
  - `start $identifier --thread <threadId> --branch $branch` once you actually start implementing. Your T3 thread ID is `threadId` in $handoff/thread-receipt.json;
  - `review $identifier --pr <url>` when the PR is review-approved and green;
  - `block $identifier --message '...'` if you need a human decision, a credential, or anything outside your authority. Then stop. The team has no Blocked state, so the ticket keeps its state; the comment is the signal.
...
- Demo/preview: if the change is user-visible, show it on the `tiptap-demo` page on a dedicated Bubble branch created from `test` (the bubble-plugin-development skill describes this). Name it after the ticket (`$identifier_lower-...`). Always set File uploads enabled explicitly. Don't delete it. In your closing comment, give its name and version id, and mark it either **preview-only** (disposable once shipped) or **demo to keep** (the maintainer must merge it into `test`). A new user-visible feature or setting is a demo to keep. Use preview-only only for checks of fixes to existing behavior that the demo page already shows.
...
## Authority
You may implement, test, commit and push your branch, open and iterate the PR, create your own Bubble feature branch, and move the ticket to In Progress or In Review (or comment a blocker).
You may NOT:
- merge the PR;
...
- delete Bubble branches or pages, or anything you didn't create.
