...
4. Pled push from the primary checkout $repo_root. It must be on `main`, clean apart from untracked files; if it isn't, `block` rather than touching someone else's work. Then `git pull --ff-only` and `pled status`. Remote-ahead or diverged means stop and `block`: understand both sides first, never force.
   ...
   - Lifecycle lab baseline (docs/wtf-256/lifecycle-lab.md). Do this after the diff check and before the label re-check, so the re-check stays right before `pled push`. The three lab checks are:
     ...
     3. `buildprint test run lifecycle_lab` (5 saved Buildprint tests on `test`).

     Setup, all under $handoff, to be reused in step 5 and removed in step 6:
     ...
     - Check 3 needs a Buildprint clone of `test` in the rico.wtf workspace. The Buildprint CLI's auth (`~/.buildprint`) is shared with other sessions, so never run `buildprint link` or `buildprint profile switch` against it. Run every rico.wtf Buildprint command with `HOME` set to a private directory `$handoff/buildprint-home` (mode 700). Link that home once with the token from `op://Dev/buildprint/workspace_ricowtf` (never print it). Then clone `tiptap-plugin` `test` into `$handoff/lab-buildprint`.
     - Record each sub-check's result, not just each script's exit code: the `ok`/`FAIL <name>` lines of checks 1 and 2, and the pass/error per test key of check 3 (`buildprint test status <runId>` for each run). Save the raw output under $handoff.
...
6. Clean up the task's worktree under $worktree_root. First check that the fix session's T3 thread isn't running and that the worktree has no uncommitted changes. Back up anything untracked to $handoff, then `git worktree remove` (no --force). Keep the Git branches.
   - Also remove the step 4 lab worktree (`git worktree remove $handoff/lab-worktree`) and delete `$handoff/lab-buildprint` and `$handoff/buildprint-home`. Keep the saved lab results.
...
8. `linear-ticket done $identifier --pr <url> --evidence '<merge sha, pled In sync, preview checks, lab checks before → after, e.g. bubble-lab 20/20 → 20/20, demo 3/3 → 3/3, buildprint 5/5 → 5/5>'`, then one final comment: what shipped, and anything left for the maintainer (for example a demo branch to merge).
