...
await scenario("save button: the saved value comes back as Initial content and is reloaded", async () => {
    const h = await makeHarness();
    h.type(" edited");
    h.advance(300);
    ...
    assert.equal(h.editor().getHTML(), saved, "the reload shows the text already on screen");
    h.advance(300);
    assert.equal(h.count("contentUpdated"), 0, "reloading identical text fires nothing");

    // Typing between the click and the reload is replaced by the saved text.
    h.type(" newer");
    h.events.length = 0;
    await h.change({ initialContent: saved.replace("</p>", " again</p>") });
    h.advance(300);
    assert.equal(h.count("contentUpdated"), 1, "reloading different text fires Content updated once");
    assert.equal(h.editor().getHTML(), saved.replace("</p>", " again</p>"));
    assert.doesNotMatch(h.editor().getHTML(), /newer/);
