...
// 2: KaTeX itself is not bundled.
assert.ok(!distSource.includes("KaTeX parse error"), "dist.js does not contain KaTeX");

async function makeEditor(overrides = {}, { autoBinding = false } = {}) {
...
const [css, script] = [katexAssets().find(item => item.tagName === "LINK"), katexAssets().find(item => item.tagName === "SCRIPT")];
assert.match(css?.href || "", /^https:\/\/cdn\.jsdelivr\.net\/npm\/katex@0\.16\.29\/dist\/katex\.min\.css$/, "pinned KaTeX stylesheet");
assert.match(css.integrity, /^sha384-/, "stylesheet has SRI");
assert.equal(css.crossOrigin, "anonymous");
assert.ok(!script || /katex@0\.16\.29\/dist\/katex\.min\.js$/.test(script.src), "pinned KaTeX script");
await settle();
assert.equal(katexAssets().filter(item => item.tagName === "SCRIPT").length, 0, "a failed script is removed so a later editor retries");
...
function allowKatexLoad() {
    ...
    document.head.appendChild = node => {
        if (node.tagName === "SCRIPT" && /katex/.test(node.src)) {
            assert.match(node.integrity, /^sha384-/, "script has SRI");
            assert.equal(node.crossOrigin, "anonymous");
            window.katex = katex; // the script ran
        }
        return append(node);
    };
}
