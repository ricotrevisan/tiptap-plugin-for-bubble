...
// Rebuilt, torn down, and one formula KaTeX throws on (too deeply nested), all
// before KaTeX arrives.
const rebuilt = await makeEditor({ initialContent: MATH_HTML });
await rebuilt.change({ ext_find_replace: true });
const tornDown = await makeEditor({ initialContent: MATH_HTML });
tornDown.close();
const deep = "{".repeat(20000) + "x" + "}".repeat(20000);
const throwing = await makeEditor({ initialContent: `<p><span data-type="inline-math" data-latex="${deep}"></span> and <span data-type="inline-math" data-latex="y^2"></span></p>` });
...
detached.close();
assert.ok(rebuilt.formulaElement(0).querySelector(".katex"), "an editor rebuilt before KaTeX arrived is typeset");
assert.ok(rebuilt.formulaElement(1).querySelector(".katex-display"));
rebuilt.close();
assert.ok(throwing.formulaElement(1).querySelector(".katex"), "a formula KaTeX throws on doesn't stop the others");
assert.ok(throwing.formulaElement(0).classList.contains("inline-math-error"), "the throwing formula is marked as an error");
assert.equal(throwing.formulaElement(0).textContent, deep, "and shows its LaTeX");
throwing.close();
