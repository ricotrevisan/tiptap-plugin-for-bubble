...
const localRef = args.find(arg => arg.startsWith('--local-initialize='))?.split('=')[1];
...
try {
  ...
  await page.waitForTimeout(1500);
  const popupZ = await page.evaluate(() => Number(getComputedStyle(document.querySelector('.bubble-element.Popup')).zIndex));
  await select(notion);
  state = await menu();
