...
// page (built on Bubble branch wtf-256-lab, merged into test). Checks what
...
const { url, localRef } = parseArgs('https://tiptap-plugin.bubbleapps.io/version-test/lifecycle-lab');
...
try {
  ...
  // A Floating Menu button that ends the empty line hides the menu while focus
  // is on the button; the action must still run once and publish.
  await page.locator('#lab-input').click();
  const errors = [];
  page.on('pageerror', error => errors.push(error.message));
  const last = editor('Editor A.').locator('> *').last();
  await last.click();
  await page.waitForTimeout(300);
  await page.keyboard.press('End');
  await page.keyboard.press('Enter');
  await page.waitForTimeout(900);
  const fa = await menuState(page, 'labFloatA');
  check('floating menu A shows on an empty line', fa.visible && fa.hitsMenu, fa);
  const divider = await page.locator('#lab-a-divider').boundingBox();
  await page.mouse.click(divider.x + divider.width / 2, divider.y + divider.height / 2);
  await page.waitForTimeout(1500);
  const published = await page.locator('#lab-a-html').textContent();
  check('the Divider workflow runs once and Content (HTML) includes the rule',
    await counter('lab-count-a-hr') === 'A Divider: 1' && await editor('Editor A.').locator('hr').count() === 1 && published.includes('<hr>') && errors.length === 0,
    { count: await counter('lab-count-a-hr'), published: published.slice(0, 120), errors });
  ...
  const popupZ = await page.evaluate(() => {
    ...
    if (!open) throw new Error('"Open popup" did not open the popup');
