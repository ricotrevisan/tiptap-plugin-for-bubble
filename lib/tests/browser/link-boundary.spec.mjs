...
async function selectWord(page, word) {
  ...
  await expect.poll(() => page.evaluate(() => link.h.instance.data.editor.view.hasFocus())).toBe(true);
}
...
test('Set link with nothing selected links the text typed next, until the caret moves', async ({ page }) => {
  ...
  await page.evaluate(() => {
    link.h.instance.data.editor.commands.focus(14);
    link.h.action('setLink', { url: 'https://example.com' });
  });
...
test('autolinked URLs stop at the space the user typed', async ({ page }) => {
  await page.evaluate(() => link.h.instance.data.editor.commands.focus('start'));
  await caretAt(page, 1);
