...
test('typing publishes coherent event snapshots and repeated update preserves editor/content', async ({ page }) => {
  ...
  expect(await page.evaluate(() => first.autobindings)).toEqual([]);
...
test('extension rebuild preserves draft; teardown and update remount independently', async ({ page }) => {
  ...
  await page.evaluate(() => first.h.instance.data.teardownEditor('browser lifecycle regression'));
...
test('keyboard and click mention selection publish the mention before firing Mention created once', async ({ page }) => {
  const created = () => page.evaluate(() => first.h.events.filter(e => e.name === 'mention_created').map(e => ({
    id: e.states.mentioned_id, label: e.states.mentioned_label, trigger: e.states.mentioned_trigger_char,
  })));
  ...
  await expect(page.locator('.tippy-content')).toContainText('Ada Lovelace');
  await page.keyboard.press('Enter');
  await expect(page.locator('#first [data-type="mention"]')).toHaveCount(1);
  expect(await created()).toEqual([{ id: 'ada', label: 'Ada Lovelace', trigger: '@' }]);
  await page.keyboard.type(' @Gr');
  await page.locator('.tippy-content .item', { hasText: 'Grace Hopper' }).click();
  await expect(page.locator('#first [data-type="mention"]')).toHaveCount(2);
  expect(await created()).toEqual([
    { id: 'ada', label: 'Ada Lovelace', trigger: '@' },
    { id: 'grace', label: 'Grace Hopper', trigger: '@' },
  ]);
  await page.locator('#first .tiptap').focus();
  await page.keyboard.press('ControlOrMeta+z');
  await expect(page.locator('#first [data-type="mention"]')).not.toHaveCount(2);
