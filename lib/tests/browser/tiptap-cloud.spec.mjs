...
test.skip(!appId || !secret, 'Set TIPTAP_CLOUD_APP_ID and TIPTAP_CLOUD_SECRET to run against Tiptap Cloud');
...
test('C2 a token Tiptap Cloud rejects gives up after five attempts and recovers with a valid token', async ({ browser }) => {
  ...
  const document = `wtf256-lab-${randomUUID()}`;
  const errors = [];
  const { context, page } = await open(browser, document, jwt(document, 'not-the-server-secret'), 'Ada', errors);
  try {
