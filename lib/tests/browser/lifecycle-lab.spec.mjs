...
test('L8 moving focus to another editor or an input hides the first editor\'s menus', async ({ page }) => {
  ...
  // A non-focusable div button (like a clickable Bubble group or icon) runs its action and keeps the menu.
...
// Wait until no write is in flight, the plugin has no queued save or repair,
// and nothing new is published for a while.
const settled = async (page, name, quiet = 1500) => {
  let last = -1;
  await expect.poll(async () => {
    // One evaluation, so a repair cannot fire between the two reads.
    const { writes, pending } = await page.evaluate(name => ({
      writes: editors[name].store.writes.length,
      pending: editors[name].store.pending() + (editors[name].h.instance.data._autobindingSave.pending ? 1 : 0),
    }), name);
    const stable = pending === 0 && writes === last;
    last = writes;
    return stable;
...
test('L11 autobinding converges with out-of-order writes, survives reload, and a menu action saves once', async ({ page }) => {
  test.slow();
  // The first write finishes last: its stale value lands in the record after
  // the newest one (WTF-260), and only a corrective write can fix it.
  await bound(page, 'a', { records: { A: '<p>Alpha</p>', B: '<p>Bravo</p>' }, current: 'A', writeDelays: [2500, 100, 100, 100],
  ...
  expect(saved.writes.map(write => write.value), `the stale first write is followed by a corrective write: ${JSON.stringify(saved)}`).toEqual([
    '<p>Alpha one</p>', '<p>Alpha one two</p>', '<p>Alpha one two three</p>', '<p>Alpha one two three</p>',
  ]);
  expect(saved.records.A, `the stored record matches the editor after writes finish out of order: ${JSON.stringify(saved)}`).toBe(editorHtml);
...
test('L14 a rejected collaboration token gives up after five attempts, releases everything, and recovers with a valid token', async ({ page }) => {
  ...
  const server = new Server({ port: 0, address: '127.0.0.1', quiet: true, async onAuthenticate({ token }) {
    if (token !== 'good-token') { rejected++; throw new Error('rejected'); }
  } });
  await server.listen();
  try {
    ...
    expect(rejected, 'the server saw exactly five attempts').toBe(5);
...
test('L15 a menu action that hides its own menu while focus is inside it still publishes the edit', async ({ page }) => {
  // "+ → Divider": a non-focusable div button in the Floating Menu inserts a
  // horizontal rule. The line is no longer empty, so Tiptap hides the menu
  // while focus is still on the menu wrapper.
  await make(page, 'a', { ...floating('fm-a'), ext_horizontalrule: true, update_delay: 0 });
  await emptyLine(page, 'a');
  await expectShown(page, '#fm-a');
  await page.locator('#fm-a .div-button').click();
  await expect(page.locator('#host-a .tiptap hr')).toHaveCount(1);
  expect(await counts(page)).toEqual({ 'a:hr': 1 });
  // (The caret lands on a new empty line after the rule, so the menu may show again.)
  // The edit reached Bubble: published content equals the editor.
  await expect.poll(() => page.evaluate(() => editors.a.h.states.contentHTML === editors.a.h.instance.data.editor.getHTML())).toBe(true);
  expect(await page.evaluate(() => editors.a.h.states.contentHTML)).toContain('<hr>');
});
