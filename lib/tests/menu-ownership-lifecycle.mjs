...
// A shown menu sits in its own layout-free portal in <body> (WTF-256).
const shownInPortal = (menu) => {
    const portal = menu.parentNode?.parentNode;
    return portal?.parentNode === document.body && portal.hasAttribute("data-tiptap-menu-portal");
};
const portals = () => document.querySelectorAll("[data-tiptap-menu-portal]").length;
...
for (const type of ["bubbleMenu", "floatingMenu"]) {
    for (const visible of [false, true]) {
        ...
        await wait(80);
        assert.equal(shownInPortal(menu), true);
        assert.equal(portals(), 1);
        if (!visible) {
            editor.view.dispatch(editor.state.tr.setMeta(type, "hide"));
            await wait(80);
            assert.equal(portals(), 0, "hiding removes the portal");
        }
        ...
        assert.equal(portals(), 0, "teardown removes the portal");
        await wait(100);
        ...
        await wait(80);
        assert.equal(shownInPortal(menu), true, "rebuild reuses the same node");
...
await change(c, { collab_active: true, collabProvider: "tiptap", collab_app_id: "fixture", collab_jwt: "fixture", collab_doc_id: "document-b" });
