...
function runFind(harness, overrides = {}) {
    actions.find(harness.instance, {
        search_term: "alpha",
        direction: "next",
...
// Search options are passed through the Bubble action, and empty searches clear state.
h.instance.data.editor.commands.setContent("<p>Alpha alpha alphabet</p>");
runFind(h, { search_term: "Alpha", case_sensitive: true });
assert.equal(findReplaceState(h).matchCount, 1);
assert.equal(findReplaceState(h).caseSensitive, true);
runFind(h, { search_term: "alpha", whole_word: true });
assert.equal(findReplaceState(h).matchCount, 2);
assert.equal(findReplaceState(h).wholeWord, true);
runFind(h, { search_term: "^Alpha|alphabet$", use_regex: true });
assert.equal(findReplaceState(h).matchCount, 2);
assert.equal(findReplaceState(h).useRegex, true);
runFind(h, { search_term: "(", use_regex: true });
assert.equal(findReplaceState(h).matchCount, 0);
runFind(h, { search_term: "" });
assert.equal(findReplaceState(h).searchTerm, "");
assert.equal(findReplaceState(h).matchCount, 0);
...
console.log(JSON.stringify({
    ...
    searchOptionsAndClearWork: true,
