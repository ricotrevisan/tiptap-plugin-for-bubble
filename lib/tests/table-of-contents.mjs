...
async function makeHarness(propertyOverrides = {}, fitHeight = false, parentElement = document.body) {
...
function tableOfContentsExtension(harness) {
    return harness.instance.data.editor.extensionManager.extensions
        .find((extension) => extension.name === "tableOfContents");
}

function tableOfContentsStorage(harness) {
    return harness.instance.data.editor.storage.tableOfContents;
}
...
const enabledTableOfContents = tableOfContentsExtension(enabled);

// Fixed-height editors register their listener on the canvas before content overflows.
assert.equal(enabledTableOfContents.options.scrollParent() === enabled.instance.canvas[0], true);
enabled.instance.canvas[0].scrollTop = 25;
enabled.instance.canvas[0].dispatchEvent(new window.Event("scroll"));
assert.equal(tableOfContentsStorage(enabled).scrollPosition, 25);

// Fit-height editors register their listener on the window instead.
const fitHeight = await makeHarness({ ext_table_of_contents: true }, true);
const fitHeightTableOfContents = tableOfContentsExtension(fitHeight);
assert.equal(fitHeightTableOfContents.options.scrollParent() === window, true);
Object.defineProperty(window, "scrollY", { value: 30, configurable: true });
window.dispatchEvent(new window.Event("scroll"));
assert.equal(tableOfContentsStorage(fitHeight).scrollPosition, 30);
fitHeight.instance.data.teardownEditor("test");

// An overflow-enabled max-content wrapper expands with the editor, so the page still scrolls.
const expandingWrapper = document.createElement("div");
expandingWrapper.style.height = "max-content";
expandingWrapper.style.overflowY = "auto";
document.body.appendChild(expandingWrapper);
const expandingFitHeight = await makeHarness({ ext_table_of_contents: true }, true, expandingWrapper);
assert.equal(tableOfContentsExtension(expandingFitHeight).options.scrollParent() === window, true);
expandingFitHeight.instance.data.teardownEditor("test");

// The resolved ancestor stays fixed for registration, event handling, and cleanup.
const outerScroller = document.createElement("div");
outerScroller.style.height = "400px";
outerScroller.style.overflowY = "auto";
const innerScroller = document.createElement("div");
innerScroller.style.height = "200px";
innerScroller.style.overflowY = "auto";
outerScroller.appendChild(innerScroller);
document.body.appendChild(outerScroller);
const nestedFitHeight = await makeHarness({ ext_table_of_contents: true }, true, innerScroller);
const nestedTableOfContents = tableOfContentsExtension(nestedFitHeight);
assert.equal(nestedTableOfContents.options.scrollParent() === innerScroller, true);
innerScroller.style.overflowY = "visible";
assert.equal(nestedTableOfContents.options.scrollParent() === innerScroller, true);
const nestedStorage = tableOfContentsStorage(nestedFitHeight);
nestedFitHeight.instance.data.teardownEditor("test");
const positionAfterTeardown = nestedStorage.scrollPosition;
innerScroller.scrollTop = 45;
innerScroller.dispatchEvent(new window.Event("scroll"));
assert.equal(nestedStorage.scrollPosition, positionAfterTeardown);
...
let scrollOptions;
const target = document.getElementById(outline[1].id);
