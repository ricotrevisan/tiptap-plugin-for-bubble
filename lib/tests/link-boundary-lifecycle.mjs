...
async function makeEditor(overrides = {}) {
    ...
    const h = {
        ...
        backspace() {
            ...
            h.editor.view.dispatch(h.editor.state.tr.delete(from - 1, from));
        },
        press(key) {
            const event = new window.KeyboardEvent("keydown", { key, bubbles: true, cancelable: true });
            return h.editor.view.someProp("handleKeyDown", handler => handler(h.editor.view, event));
        },
...
for (const autolink of [true, false]) {
    ...
    h.select(14);
    assert.equal(h.editor.isActive("link"), false, `${label}: the caret at the right edge is outside the link`);
    h.type(" now");
...
// 5b. Backspace while typing a new link keeps it going; so does retyping.
{
    const h = await makeEditor();
    h.select(14);
    h.setLink();
    ...
    h.backspace();
    h.type("e");
    assert.equal(h.linked(), " guide", "fixing a typo keeps the new link");
    h.backspace();
    h.backspace();
    h.backspace();
    h.backspace();
    ...
    h.backspace();
    h.type("x");
    assert.equal(h.linked(), "", "deleting all of the new link ends it");
    h.close();
}

// 5c. At the end of a paragraph, ArrowRight ends the link being typed.
{
    const h = await makeEditor();
    ...
    h.setLink();
    h.type(" docs");
    assert.equal(h.press("ArrowRight"), true, "ArrowRight is handled at the end of a paragraph");
    h.type(" now");
    assert.equal(h.linked(), " docs", "text after ArrowRight is plain");
    h.select(end);
    assert.ok(!h.press("ArrowRight"), "without a link being typed, ArrowRight is left to the browser");
    h.close();
}

// 5d. An input rule inside the link being typed keeps it going. A
//     replacement reaching back before it, or IME composition, ends it.
{
    const h = await makeEditor();
    h.select(14);
    h.setLink();
    h.type(" (c");
    const { from } = h.editor.state.selection;
    h.editor.view.dispatch(h.editor.state.tr.insertText("©", from - 2, from));
    h.type("x");
    assert.equal(h.linked(), " ©x", "an input rule inside the new link keeps it going");
    h.close();
}
{
    const h = await makeEditor();
    ...
    h.type("ab");
    const { from } = h.editor.state.selection;
    h.editor.view.dispatch(h.editor.state.tr.insertText("Z", from - 5, from));
    h.type("q");
    assert.equal(h.editor.getText(), "Read the dZq today");
    assert.equal(h.linked(), "Z", "a replacement reaching before the new link ends it");
    h.close();
}
{
    const h = await makeEditor();
    h.select(14);
    h.setLink();
    h.type("y", { composition: 1 });
    h.type("z", { composition: 1 });
    assert.equal(h.linked(), "y", "IME composition is left to ProseMirror");
    h.close();
}
{
    const thing = values => ({ get: key => values[key] });
    const people = [thing({ name: "Ada", uid: "ada" })];
    const h = await makeEditor({
        ext_mention: true, mention_triggerChar: "@", mention_field_label: "name", mention_field_id: "uid",
        mention_list: { length: () => people.length, get: (start, count) => people.slice(start, start + count) },
    });
    h.select(14);
    h.setLink();
    h.type(" a");
    h.editor.commands.insertContent({ type: "mention", attrs: { id: "ada", label: "Ada" } });
    h.type("b");
    const mention = [];
    h.editor.state.doc.descendants(node => { if (node.type.name === "mention") mention.push(node.marks.map(m => m.type.name)); });
    assert.deepEqual(mention, [[]], "a mention inserted while typing a link is not linked");
    assert.equal(h.linked(), " a", "inserting a mention ends the new link");
    h.close();
}
