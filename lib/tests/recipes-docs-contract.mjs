...
// Recipes run on the development demo page (login tippy / tappy). The README
// and Marketplace listing use the public demo page instead (WTF-256).
const canonicalDemo = "https://tiptap-plugin.bubbleapps.io/version-test/tiptap-demo";
const publicDemo = "https://nocode-to-knowcode.bubbleapps.io/version-test/tiptap";
const oldDemos = ["tiptap-demo.bubbleapps.io/version-test/doc/demo", "version-test/doc-nobind/demo"];

const readme = read("README.md");
assert.ok(readme.includes(publicDemo), "README links the public demo page");
assert.ok(readme.includes(canonicalDemo), "README says where the recipes run");
assert.ok(oldDemos.every((old) => !readme.includes(old)), "README no longer links an old demo");
assert.ok(readme.includes("(docs/recipes.md)"), "README links the recipes");

const pluginMeta = json("src/plugin.json").meta_data;
assert.equal(pluginMeta.demo_page, publicDemo, "Marketplace demo page is the public demo");
assert.ok(pluginMeta.description.includes(publicDemo), "plugin description links the public demo");
assert.ok(oldDemos.every((old) => !pluginMeta.description.includes(old)), "plugin description no longer links an old demo");

// The hosted Hocuspocus service moved from tiptap.rico.wtf to whoistyping.wtf.
// Plugin text uses styled Unicode letters, so compare the NFKC form.
const pluginText = ["src/plugin.json", "src/elements/tiptap-AAC/AAC.json", "src/elements/tiptap-AAC/fields.txt"]
    .map((path) => [path, read(path).normalize("NFKC")]);
for (const [path, text] of pluginText) {
    assert.ok(!text.includes("tiptap.rico.wtf"), `${path} no longer points at tiptap.rico.wtf`);
}
assert.ok(pluginMeta.description.includes("https://whoistyping.wtf"), "plugin description links whoistyping.wtf");
const collabLabel = Object.values(json("src/elements/tiptap-AAC/AAC.json").fields).find((field) => field.name === "collab_info_sub").caption.normalize("NFKC");
assert.ok(collabLabel.includes("whoistyping.wtf"), "collaboration help names whoistyping.wtf");
...
assert.ok(oldDemos.every((old) => !recipes.includes(old)), "recipes don't link an old demo");
...
// Saving a collaborative copy must wait for the shared document, not just the editor.
const collaborationRecipe = recipeSections.find((section) => /collaboration/i.test(section.title)).body;
assert.match(collaborationRecipe, /\*\*Only when\*\*[^.]*\*\*Is ready\*\*[^.]*\*\*Collaboration synced\?\*\*/,
    "Save a copy waits for Is ready and Collaboration synced?");
