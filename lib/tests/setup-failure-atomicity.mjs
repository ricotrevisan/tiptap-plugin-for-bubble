...
// 5. Liveblocks: partial setup releases the room and its subscriptions once,
//    whether the provider or the editor fails, and the room's Y.Doc once.
{
    const service = createFakeLiveblocks(tiptap.Y);
    tiptap.createClient = options => service.createClient(options);
    const properties = { collab_active: true, collabProvider: "liveblocks",
        liveblocksPublicApiKey: "pk_test", collab_doc_id: "room" };
    const listeners = () => service.history.reduce((total, session) =>
        total + Object.values(session.events).reduce((sum, source) => sum + source.size, 0), 0);

    tiptap.LiveblocksProvider = class { constructor() { throw new Error("injected Liveblocks failure"); } };
    let h = await makeHarness(properties);
    tiptap.LiveblocksProvider = real.LiveblocksProvider;
    assertFailedCleanly(h, "Liveblocks provider failure");
    assert.equal(service.left, service.entered, "room left exactly once");
    assert.equal(listeners(), 0, "room subscriptions released");
    h.close();
    assert.equal(service.left, 1);

    let documentDestroys = 0;
    tiptap.LiveblocksProvider = class extends real.LiveblocksProvider {
        constructor(room, document) {
            ...
            document.destroy = () => { documentDestroys++; destroy(); };
        }
    };
    failEditor = true;
    h = await makeHarness(properties);
    failEditor = false;
    tiptap.LiveblocksProvider = real.LiveblocksProvider;
    assertFailedCleanly(h, "editor failure after Liveblocks provider");
    assert.equal(service.left, service.entered, "room left after editor failure");
    assert.equal(service.sessions.size, 0);
    assert.equal(listeners(), 0, "provider and element room subscriptions released");
    assert.equal(documentDestroys, 1, "provider-owned Y.Doc destroyed exactly once");
    h.close();
    assert.equal(service.left, 2);
