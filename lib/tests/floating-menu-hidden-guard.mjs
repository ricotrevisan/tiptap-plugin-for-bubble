...
// Bubble's page/floating groups are top-level stacking contexts. A menu moved
// to <body> must paint above them, or it can be style-visible but visually hidden.
const bubbleFloatingGroup = document.createElement("div");
bubbleFloatingGroup.style.position = "fixed";
bubbleFloatingGroup.style.zIndex = "1505";
document.body.appendChild(bubbleFloatingGroup);
...

{
    ...
    assert.equal(
        floatEl.style.zIndex,
        "1506",
        "shown floating menu paints above Bubble's top-level stacking contexts",
    );
}
...
assert.equal(floatEl.style.zIndex, "", "hiding restores the Bubble group's original z-index");
...
console.log(
    JSON.stringify({
        ...
        visibleAboveBubbleStackingContexts: true,
