...
// 6. Connection errors are reported once until the room connects again,
//    without the raw provider message; other room errors don't use that slot.
const error = Object.assign(new Error("room room-1 secret detail"), { context: { type: "ROOM_CONNECTION_ERROR", code: 4001 } });
a.session.events.error.notify(Object.assign(new Error("feed detail"), { context: { type: "FEED_REQUEST_ERROR" } }));
a.session.events.error.notify(error);
a.session.events.error.notify(error);
assert.deepEqual(a.messages, ["Liveblocks error (FEED_REQUEST_ERROR).",
    "Liveblocks connection error (code 4001). Check the public API key and the room's permissions."]);
a.session.drop();
a.session.connect();
await settle();
a.session.events.error.notify(error);
assert.equal(a.messages.length, 3, "reported again after the room reconnects");
a.messages.length = 0;
