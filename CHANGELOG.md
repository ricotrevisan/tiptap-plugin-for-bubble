...
- New [recipes](docs/recipes.md) with short, tested setups: a Save button, autobinding while switching records, menus in reusables and repeating groups, read-only text with the editor's styles, and live collaboration with Tiptap Cloud or a Hocuspocus server such as whoistyping.wtf.
...
- Pressing Enter or Tab when no suggestion matches no longer inserts an empty mention.
...
- Switching rooms, changing collaboration settings, turning collaboration off, or resetting the element leaves the old Liveblocks room and releases its listeners and shared document exactly once.
